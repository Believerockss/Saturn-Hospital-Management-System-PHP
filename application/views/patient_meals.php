...
<div id="content" class="content">
	...
	<h1 class="page-header">
		<a onclick="showAjaxModal('<?php echo base_url(); ?>modal/popup/modal_add_patient_meal');" href="javascript:;" class="btn btn-inverse m-r-5">
			<i class="fa fa-plus"></i> Add Patient Meal
		</a>
	</h1>
	<!-- end page-header -->
	<?php else: ?>
					...
					<table id="data-table-buttons" class="table table-striped table-bordered">
						...
						<tbody>
							...
							if ($this->session->userdata('user_type') == 'staff') {
								$patient_meals = $this->security->xss_clean($this->db->get('patient_meal')->result_array());
							} else {
							...
							?>
								<tr>
									...
									<td>
										<?php
										if ($this->db->get_where('occupancy', array('occupancy_id' => $row['occupancy_id']))->row()->patient_id) {
									...
									<td>
										<?php
										if ($this->db->get_where('occupancy', array('occupancy_id' => $row['occupancy_id']))->row()->accommodation_category_id) {
											...
											if ($this->db->get_where('occupancy', array('occupancy_id' => $row['occupancy_id']))->row()->accommodation_id) {
												...
												if ($this->db->get_where('accommodation_category', array('accommodation_category_id' => $accommodation_category_id))->num_rows() > 0) {
													...
													if ($accommodation_type == 1) {
														if ($this->db->get_where('bed', array('bed_id' => $accommodation_id))->num_rows() > 0 && $this->db->get_where('bed', array('bed_id' => $accommodation_id))->row()->accommodation_id) {
