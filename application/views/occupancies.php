...
<div id="content" class="content">
	...
	<h1 class="page-header">
		<a onclick="showAjaxModal('<?php echo base_url(); ?>modal/popup/modal_add_occupancy');" href="javascript:;" class="btn btn-inverse m-r-5">
			<i class="fa fa-plus"></i> Add Occupancy
		</a>
	</h1>
	<!-- end page-header -->
	<?php else: ?>
						...
						<tbody>
							...
							?>
								<tr>
										...
										<td>
											...
											if ($this->db->get_where('bed', array('bed_id' => $row['accommodation_id']))->num_rows() > 0 && $this->db->get_where('bed', array('bed_id' => $row['accommodation_id']))->row()->accommodation_id) {
												...
												if ($this->db->get_where('bed', array('bed_id' => $row['accommodation_id']))->num_rows() > 0)
													echo 'Bed ' . $this->security->xss_clean($this->db->get_where('bed', array('bed_id' => $row['accommodation_id']))->row()->bed_number) . ' of Room ' . ($this->db->get_where('accommodation', array('accommodation_id' => $accommodation_id))->num_rows() > 0 ? $this->security->xss_clean($this->db->get_where('accommodation', array('accommodation_id' => $accommodation_id))->row()->room_number) : 'not found.');
												else
													echo 'Bed not found of Room ' . $this->db->get_where('accommodation', array('accommodation_id' => $accommodation_id))->num_rows() > 0 ? $this->security->xss_clean($this->db->get_where('accommodation', array('accommodation_id' => $accommodation_id))->row()->room_number) : 'not found.';
											} else {
										...
										<td>
											<?php 
												if ($this->db->get_where('accommodation', array('accommodation_id' => $row['accommodation_id']))->num_rows() > 0)
													echo 'Room ' . $this->security->xss_clean($this->db->get_where('accommodation', array('accommodation_id' => $row['accommodation_id']))->row()->room_number); 
												else
													echo 'Room not found.';
											?>
										</td>
									...
									else : 'Accommodation Category not founf.';
									endif; 
									?>
									<td>
										<?php
