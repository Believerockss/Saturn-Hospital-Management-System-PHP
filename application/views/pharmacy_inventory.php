...
<div id="content" class="content">
	...
	<h1 class="page-header">
		<a onclick="showAjaxModal('<?php echo base_url(); ?>modal/popup/modal_add_pharmacy_inventory');" href="javascript:;" class="btn btn-inverse m-r-5">
			<i class="fa fa-plus"></i> Add Pharmacy Inventory
		</a>
	</h1>
	<!-- end page-header -->
				...
				<div class="panel-body">
					<table id="data-table-buttons" class="table table-striped table-bordered">
						...
						<tbody>
							...
							$pharmacy_inventory = $this->security->xss_clean($this->db->get('pharmacy_inventory')->result_array());
							...
							?>
								<tr>
									...
									<td>
										<?php echo $this->security->xss_clean($this->db->get_where('setting', array('item' => 'currency'))->row()->content); ?>
										<?php echo $row['price']; ?>
									</td>
									<td>
										<?php 
											if ($this->db->get_where('unit', array('unit_id' => $row['unit_id']))->num_rows() > 0)
												echo $row['quantity'] . ' ' . $this->security->xss_clean($this->db->get_where('unit', array('unit_id' => $row['unit_id']))->row()->name); 
											else
												echo $row['quantity'];
										?>
									</td>
									...
									<td>
										<div class="btn-group">
											...
											<div class="dropdown-menu dropdown-menu-right">
												<a class="dropdown-item" onclick="showAjaxModal('<?php echo base_url(); ?>modal/popup/modal_edit_pharmacy_inventory_details/<?php echo $row['pharmacy_inventory_id']; ?>');" href="javascript:;">Edit Details</a>
												<div class="dropdown-divider"></div>
												<a class="dropdown-item" onclick="showAjaxModal('<?php echo base_url(); ?>modal/popup/modal_edit_pharmacy_inventory/<?php echo $row['pharmacy_inventory_id']; ?>');" href="javascript:;">Edit Inventory</a>
											</div>
										</div>
									</td>
								</tr>
